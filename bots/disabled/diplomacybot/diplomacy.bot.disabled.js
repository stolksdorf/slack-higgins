...
const utils = require('slack-microbots/utils.js');
const Moment = require('moment');

const Diplomacy = require('./diplomacy.game.js');
const Print = require('./diplomacy.print.js');

const ACTIONS = ['defend', 'attack', 'support', 'invest'];
const BOT_NAMES = ['higgins', 'higs', 'diplomacybot'];

let Higs = require('slack-microbots').getBotInstance({
	name : 'diplomacybot',
...
const parseMove = function(msg, playerName){
	let target;
	const action = _.find(ACTIONS, (action)=>{return msg.indexOf(action) !== -1;});
	if(action == 'attack' || action == 'support'){
		const index = _.reduce(_.words(msg), (r, part, i)=>{
...
const submitMove = function(playerName, action, target){
...
const triggerStart = function(username, message){
	let rounds = 6;
	let roundLengthMs = 1000 * 60 * 60 * 24;

	//Extract out is the user passed game params
	const params = utils.extractNumberUnits(message);
	_.each(params, (param)=>{
